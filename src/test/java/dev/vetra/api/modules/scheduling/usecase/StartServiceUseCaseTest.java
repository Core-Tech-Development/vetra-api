...
class StartServiceUseCaseTest {
    ...
    private StartServiceUseCase useCase;
    ...
    void setUp() {
        ...
        useCase = new StartServiceUseCase(appointmentRepository, ownershipValidator,
    ...
    @Test
    void shouldFailWhenNotInTransitStatus() {
        ...
        useCase.execute(APPOINTMENT_ID, CALLER, ROLES)
                .subscribe().withSubscriber(UniAssertSubscriber.create())
                .assertFailedWith(BusinessException.class, "Cannot start service");
    }

    @Test
    void shouldTransitionToInServiceWithActualStartTime() {
        var appointment = Appointment.restore(APPOINTMENT_ID, EXAM_REQUEST_ID, SPECIALIST_ID, null,
                Instant.now(), Instant.now().plusSeconds(3600),
        ...
        assertThat(result.status()).isEqualTo(AppointmentStatus.IN_SERVICE);
        assertThat(result.actualStartAt()).isNotNull();
    }
}

...
class CompleteExamUseCaseTest {
    ...
    void setUp() {
        ...
        useCase = new CompleteExamUseCase(appointmentRepository, ownershipValidator, notificationService, auditUseCase);
    }
    ...
    @Test
    void shouldFailWhenNotInServiceStatus() {
        ...
        useCase.execute(APPOINTMENT_ID, CALLER, ROLES)
                .subscribe().withSubscriber(UniAssertSubscriber.create())
                .assertFailedWith(BusinessException.class, "Cannot complete exam");
    }

    @Test
    void shouldTransitionToWaitingReportWithActualEndTime() {
        var appointment = Appointment.restore(APPOINTMENT_ID, EXAM_REQUEST_ID, SPECIALIST_ID, null,
                Instant.now(), Instant.now().plusSeconds(3600),
                Instant.now(), null, AppointmentStatus.IN_SERVICE, null, null,
                Instant.now(), Instant.now());
        ...
        assertThat(result.status()).isEqualTo(AppointmentStatus.WAITING_REPORT);
        assertThat(result.actualEndAt()).isNotNull();
    }
}

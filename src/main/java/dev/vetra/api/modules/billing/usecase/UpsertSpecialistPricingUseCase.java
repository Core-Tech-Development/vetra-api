...
@ApplicationScoped
public class UpsertSpecialistPricingUseCase {

    private static final Logger LOG = Logger.getLogger(UpsertSpecialistPricingUseCase.class);
    private final SpecialistPricingRepository repository;
    private final CachedPricingService cachedPricingService;

    @Inject
    public UpsertSpecialistPricingUseCase(SpecialistPricingRepository repository,
    ...
    public Uni<SpecialistPricing> execute(UUID specialistId, String examType, long priceCents) {
        ...
        LOG.infof("Upserting specialist pricing: specialist=%s, exam=%s, price=%d", specialistId, examType, priceCents);
        return repository.upsert(pricing)
